...
js 内置了 [Crypo](https://developer.mozilla.org/zh-CN/docs/Web/API/Crypto) 接口来实现常用的加密解密 api。
...
Server ->> +Client: Certificate 发送TSL(SSL)证书(和上条消息是一起发送的)
deactivate Server
end
par 第二个 RTT
note left of Server: 证书里含有公钥，客户端使用根证书进行证书校验
...
> 来回通信延迟([RTT](https://developer.mozilla.org/zh-CN/docs/Glossary/Round_Trip_Time)): Rount_Trip_Time
> 指数据包从发送到目的地再到返回到源站所需的总时间。可以使用 `ping` 命令来确定网络和服务器之间的 RTT。
...
64 bytes from 150.171.28.10: icmp_seq=0 ttl=115 time=96.070 ms
64 bytes from 150.171.28.10: icmp_seq=1 ttl=115 time=96.942 ms
64 bytes from 150.171.28.10: icmp_seq=2 ttl=115 time=95.569 ms
--- bing.com ping statistics ---
3 packets transmitted, 3 packets received, 0.0% packet loss
round-trip min/avg/max/stddev = 95.569/96.194/96.942/0.567 ms
```

#### 4.2 版本 1.3 的建立连接过程

TLS 1.3 是最新的协议，可能也是使用最广泛的协议。
* [图解 TCL1.3](https://cangsdarm.github.io/illustrate/tls13)，解释非常详尽。

TLS 1.3 使用 DH 算法替代 RSA，提高安全性的同时减少建立连接的步骤。

TLS 1.2 握手过程只需要 1 个 RTT。在存在已有会话的情况下，可以做到 0 RTT.
...
Client ->> +Server: 客户端发送 DH 参数 A
Server ->> Client: 服务端发送 DH 参数 B
...
在不安全信道中，传输已经加密的文件是可行的，但如果直接传输未加密的参数本身，就存在参数泄露的风险。

DH 纯粹使用**数学理论**，在不互相直接传输参数的情况下，完成参数的交换。

> DH 算法的安全性基于离散对数问题的困难性：
> * 已知p、g、A，计算 a 是困难的（即从`A = g^a mod p` 中求 a 是困难的）。同样，从 `B = g^b mod p` 中求 b 也是困难的。
> * 因此，即使攻击者截获了 A 和 B，也无法计算出共享密钥 `s = g^(ab) mod p` 中的这个值 **s**。
