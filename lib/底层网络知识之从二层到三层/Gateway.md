...
sequenceDiagram
	...
	opt 数据包: <br/>源 MAC 地址:服务器 A 的 MAC<br/>目标 MAC 地址:192.168.1.1 这个网口的 MAC<br/>源 IP 地址:192.168.1.101<br/>目标 IP 地址:192.168.4.101
	服务器A->>+路由器A: 192.168.1.101 发送数据包给 192.168.1.1
	end
	Note right of 路由器A: 192.168.1.1/24 转 192.168.56.1/24
	opt 数据包: <br/>源 MAC 地址:192.168.56.1 的 MAC<br/>目标 MAC 地址:192.168.56.2 这个网口的 MAC<br/>源 IP 地址:192.168.1.101<br/>目标 IP 地址:192.168.4.101
	路由器A->>-路由器B: 192.168.56.1 网关转发数据包到 192.168.56.2
	activate 路由器B
	end
	Note left of 路由器B: 192.168.56.2/24 转 192.168.4.1/24
	opt 数据包: <br/>源 MAC 地址:192.168.4.1 的 MAC<br/>目标 MAC 地址:192.168.4.101 这个网口的 MAC<br/>源 IP 地址:192.168.1.101<br/>目标 IP 地址:192.168.4.101
	路由器B->>服务器B: 192.168.4.1 网关转发到服务器B 192.168.4.101
	deactivate 路由器B
	end
	activate 服务器B
	服务器B->>服务器B: 确认 MAC 地址与本机地址符合，接收数据包
	deactivate 服务器B
```
...
sequenceDiagram
	...
	participant 服务器B
	Note over 服务器A,路由器A: 发送给网关
	opt 数据包: <br/>源 MAC 地址:服务器 A 的 MAC<br/>目标 MAC 地址:192.168.1.1 这个网口的 MAC<br/>源 IP 地址:192.168.1.101<br/>目标 IP 地址:192.168.56.2
	服务器A->>+路由器A: 192.168.1.101 发送数据包给 192.168.1.1
	end
	Note right of 路由器A: 192.168.1.1/24 转 192.168.56.1/24
	opt 数据包: <br/>源 MAC 地址:192.168.56.1 的 MAC<br/>目标 MAC 地址:192.168.56.2 MAC<br/>源 IP 地址:192.168.56.1<br/>目标 IP 地址:192.168.56.2
	路由器A->>-路由器B: 192.168.56.1 网关转发数据包到 192.168.56.2
	end
	activate 路由器B
	Note left of 路由器B: 192.168.56.2/24 转 192.168.1.1/24
	opt 数据包: <br/>源 MAC 地址:192.168.1.1 的 MAC<br/>目标 MAC 地址:192.168.1.101 MAC<br/>源 IP 地址:192.168.56.1<br/>目标 IP 地址:192.168.1.101
